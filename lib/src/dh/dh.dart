...
class DhKey {
  ...
  int computeKey(DhKey other) {
    ...
    if (other._publicKey <= 0 || other._publicKey > _group.prime.toInt()) {
    ...
    return (other._publicKey ^ _privateKey) % _group.prime.toInt();
  }
}
...
class DhGroup {
  ...
  DhKey generateKey() {
    int private = randomInt(prime.toInt() - 1) + 1;
    int public = (generator ^ private) % prime.toInt();
